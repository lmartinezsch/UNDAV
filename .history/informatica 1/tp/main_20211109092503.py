...
def saveData():
    with open('/var/www/UNDAV/informatica 1/tp/data.csv', 'rb') as f:
    ...
    writer = csv.writer(open("/var/www/UNDAV/informatica 1/tp/data.csv", 'w'))
    '''for row in data:
        if counter[row[0]] >= 4:
            writer.writerow(row)'''
    entries = ["hola1", "hola2"]
    ...
    dataCsv = open('/var/www/UNDAV/informatica 1/tp/data.csv', 'w')

    with dataCsv:
        
        fnames = ['first_name', 'last_name']
        writer = csv.DictWriter(dataCsv, fieldnames=fnames)    
